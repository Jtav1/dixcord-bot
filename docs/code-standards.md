...
- **Named-function handlers**: `utilities/triggerResponseFunctions.js` keys handler functions by `function_name`, looked up by the trigger-response flow — follow this pattern for similar "string key → handler function" extension points rather than a growing `switch`.
