/** Server-side webapi JWT for the webadmin proxy (not bundled for the browser). */
...
/**
 * Read webapi URL and admin credentials from process.env.
...
/**
 * Ensure admin service account credentials are configured.
...
export function requireAdminCredentials() {
  ...
  if (!username || !password) {
    throw new Error(
      "ADMIN_USERNAME and ADMIN_PASSWORD must be set for the API proxy.",
    );
  }
  return { webapiUrl, username, password };
}
...
async function loginForToken() {
  const { webapiUrl, username, password } = requireAdminCredentials();
...
export async function webapiAuthProxyMiddleware(req, res, next) {
  ...
  try {
    ...
    console.error("webadmin webapi auth failed:", err);
    res.statusCode = 502;
...
export async function warmWebapiAuth() {
  ...
  console.log("webadmin webapi auth token ready");
}
