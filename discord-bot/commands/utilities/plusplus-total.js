...
const execute = async (interaction) => {
  ...
  if (word && !user) {
    ...
    outStr +=
      "Total score for " + "<@" + user.id + "> is " + totalUserScore[0].total;
  } else if (user && word) {
    //prettier-ignore
    outStr += "Wow two at once. Score for " + word + " is " + totalWordScore[0].total +
      " and score for " + "<@" + user.id + "> is " + totalUserScore[0].total;
  } else {
