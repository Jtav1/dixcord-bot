...
- **Trigger responses** – If the message matches a trigger string, replies using the trigger’s `selection_mode`:
  ...
  - `weighted` — weighted random; a response may optionally carry a `response_function` function name, in which case the bot dispatches to that function (defined in [`utilities/triggerResponseFunctions.js`](utilities/triggerResponseFunctions.js), keyed by `function_name`) instead of auto-replying
