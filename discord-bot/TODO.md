...
| [ x ]      | [--]           | Emoji frequency (counts)            | `countEmoji`, `countRepost`, `uncountRepost` (and related reads) are DB writes/reads.                                         | On reaction add/remove, send (emoji, user, message, type) to API; bot applies result (e.g. no reply).      |
