...
export async function getDatabaseStatistics() {
  const [rows] = await db.query(
    `SELECT
      ...
      (SELECT COALESCE(SUM(frequency), 0) FROM guild_emojis WHERE type = 'emoji' OR type IS NULL) AS emoji_usage_total,
      (SELECT COALESCE(SUM(frequency), 0) FROM guild_emojis WHERE type = 'sticker') AS sticker_usage_total,
