...
- **Chatlog (cleanLog)** – Sanitizes every message (strip mentions, URLs, PII, filter words), appends to per-channel log files.
...
| Status | Feature | Why it fits the API | What stays in bot |
|---------|---------|---------|---------|
| [--] | Take a look at this | Link selection (common/rare), rate limit, spam logic, and `incrementTakeALookLink` are pure logic + DB. | Detect trigger phrase, send reply with returned URL (or "No spam"). |
| [--] | Fortune teller (8-ball) | "Pick random fortune" + `incrementFortune` is logic + DB. | Detect @bot + `?`, send returned fortune text. |
| [--] | Twitter/social link fixer | URL parsing and domain swap rules are stateless logic; config (e.g. `twitter_fix_enabled`) can be read by API. | Detect links + keywords, send returned "fixed link" string. |
...
| [--] | Chatlog (cleanLog) | Sanitization rules and "append to log" (or "store in DB" if you move logs to API) are pure logic + I/O. | On message: send content + channel id; API returns "ok" or stores; bot doesn't need log content. |
